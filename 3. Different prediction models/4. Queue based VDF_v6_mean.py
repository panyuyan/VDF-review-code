...
if len(valid_days) < 5:
    ...
    mu_agg    = float(np.nanmean(train_df["mu"].values)) if not train_df.empty else np.nan
    gamma_agg = float(np.nanmean(train_df["gamma"].values)) if not train_df.empty else np.nan
